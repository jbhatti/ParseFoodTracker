    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        case "ShowDetail":
            ...
            guard let selectedMealCell = sender as? MealTableViewCell else {
                fatalError("Unexpected sender: \(sender ?? "test")")
            }
        ...
        default:
            fatalError("Unexpected Segue Identifier; \(segue.identifier ?? "test")")
        }
    }
...
//    }
    ...
    private func fetchMeals(completion: @escaping (PFObject) -> () ) {
        let query = PFQuery(className: "Meal")
        query.findObjectsInBackground { (meals: [PFObject]?, error: Error?) in
            if let error = error {
    ...
    private func loadMeals() -> [Meal]?  {
        ...
        query.findObjectsInBackground { (meals: [PFObject]?, error: Error?) in
            ...
            guard let allMeals = meals as? [Meal] else {
                return
            }
            var mealsArray:[Meal] = []
